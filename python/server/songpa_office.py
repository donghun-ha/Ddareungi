...
# 송파구청 실시간 주차대수 가져오기
def get_station():
    url = "http://openapi.seoul.go.kr:8088/496c726e54746c733132325669414e57/json/bikeList/1/1000"
    response = requests.get(url)
    data = response.json()
# 롯데타워 잠실역 2번출구 따릉이 스테이션 정보 찾기
    target_station = None
    for station in data['rentBikeStatus']['row']:
        if "ST-1409" in station['stationId']:
    ...
    if target_station:
        ...
        return target_station['parkingBikeTotCnt'], target_station['rackTotCnt'] #현재 , 거치대 
    else:
        print("1278. 송파구청 교차로 데이터 없음")
